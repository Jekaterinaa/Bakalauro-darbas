...
BTCtrain <- as.data.frame(modelData(BTCmodel,
                                    data.window=c('2018-03-05', '2023-02-13')))
BTCeval <- na.omit(as.data.frame(modelData(BTCmodel,
                                           data.window=c('2023-02-14', '2023-03-02'))))
...
preds <- slidingWindowTest(model, BTCform, BTCtrain, BTCeval, relearn.step=7)
...
preds <- growingWindowTest(model, ETHform, ETHtrain, ETHeval, relearn.step=7)
signals <- factor(preds, levels=1:3, labels=c('s', 'h', 'b'))
...
# Slide, earth, pol2, 7
model <- learner("MC.earth")
preds <- slidingWindowTest(model, BNBform, BNBtrain, BNBeval, relearn.step=7)
signals <- factor(preds, levels=1:3, labels=c('s', 'h', 'b'))
...
preds <- growingWindowTest(model, XRPform, XRPtrain, XRPeval, relearn.step=7)
signals <- factor(preds, levels=1:3, labels=c('s', 'h', 'b'))
...
# Grow, earth, pol2, 7
model <- learner("MC.earth")
preds <- growingWindowTest(model, ADAform, ADAtrain, ADAeval, relearn.step=7)
signals <- factor(preds, levels=1:3, labels=c('s', 'h', 'b'))
...
# Slide, svmC, pol2, 7
model <- learner("MC.svmC", list(maxit=100, linout=T, trace=F, size=10, decay=0.001))
preds <- slidingWindowTest(model, BCHform, BCHtrain, BCHeval, relearn.step=7)
signals <- factor(preds, levels=1:3, labels=c('s', 'h', 'b'))
...
# Slide, earth, pol2, 7
model <- learner("MC.earth")
preds <- slidingWindowTest(model, OMGform, OMGtrain, OMGeval, relearn.step=7)
signals <- factor(preds, levels=1:3, labels=c('s', 'h', 'b'))
